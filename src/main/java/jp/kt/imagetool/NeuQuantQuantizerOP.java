 ...
 */
class NeuQuantQuantizerOP implements BufferedImageOp {

	public BufferedImage filter(BufferedImage src, BufferedImage dest) {

		try {
			NeuQuant nq = new NeuQuant(src, src.getWidth(), src.getHeight());
			nq.init();
	...
	public BufferedImage createCompatibleDestImage(BufferedImage src,
		...
		return new BufferedImage(destCM, destCM.createCompatibleWritableRaster(
				width, height), destCM.isAlphaPremultiplied(), null);
	}
