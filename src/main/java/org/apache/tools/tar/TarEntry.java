...
public class TarEntry implements TarConstants {
	 ...
	 *            The tar entry header buffer to fill in.
	 * @param encoding
	 *            encoding to use when writing the file name.
	 * @param starMode
	 *            whether to use the star/GNU tar/BSD tar extension for numeric
	 *            fields if their value doesn't fit in the maximum size of
	...
	private void parseTarHeader(byte[] header, ZipEncoding encoding,
		...
		case FORMAT_POSIX:
		default: {
			String prefix = oldStyle ? TarUtils.parseName(header, offset,
					PREFIXLEN) : TarUtils.parseName(header, offset, PREFIXLEN,
					encoding);
	 ...
	 *
	 * @param header
	 *            The tar entry header buffer to evaluate the format for.
	 * @return format type
	 */
	 ...
	 */
	private static boolean isEqual(final byte[] buffer1, final int offset1,
			final int length1, final byte[] buffer2, final int offset2,
			final int length2, boolean ignoreTrailingNulls) {
