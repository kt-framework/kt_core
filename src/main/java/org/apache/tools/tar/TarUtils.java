...
public class TarUtils {
	...
	static final ZipEncoding DEFAULT_ENCODING = ZipEncodingHelper
			.getZipEncoding(null);
	 ...
	 *
	 * <p>
	 * Leading spaces are ignored. The buffer must contain a trailing space or
	 * NUL, and may contain an additional trailing space or NUL.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * To work-around some tar implementations that insert a leading NUL this
	 * method returns 0 if it detects a leading NUL since Ant 1.9.
	 * </p>
	 *
	 * @param buffer
	 ...
	 * @param length
	 *            The maximum number of bytes to parse - must be at least 2
	 *            bytes.
	 * @return The long value of the octal string.
	 * @throws IllegalArgumentException
	 *             if the trailing space/NUL is missing or if a invalid byte is
	 *             detected.
	 */
	public static long parseOctal(final byte[] buffer, final int offset,
			final int length) {
		long result = 0;
		...
		if (trailer == 0 || trailer == ' ') {
			...
			throw new IllegalArgumentException(exceptionMessage(buffer, offset,
					length, end - 1, trailer));
		}
		...
		for (; start < end; start++) {
			...
			if (currentByte < '0' || currentByte > '7') {
				throw new IllegalArgumentException(exceptionMessage(buffer,
						offset, length, start, currentByte));
			}
	...
	/**
	 * Compute the value contained in a byte buffer. If the most significant bit
	 * of the first byte in the buffer is set, this bit is ignored and the rest
	 * of the buffer is interpreted as a binary number. Otherwise, the buffer is
	 ...
	 * @throws IllegalArgumentException
	 *             if the trailing space/NUL is missing or an invalid byte is
	 *             detected in an octal number, or if a binary number would
	...
	/**
	 * Parse a boolean byte from a buffer. Leading spaces and NUL are ignored.
	...
	/**
	 * Parse an entry name from a buffer. Parsing stops when a NUL is found or
	...
	/**
	 * Copy a name into a buffer. Copies characters from the name into the
	 * buffer starting at the specified offset. If the buffer is longer than the
	 * name, the buffer is filled with trailing NULs. If the name is longer than
	 * the buffer, the output is truncated.
	 *
	 * @param name
	 ...
	 */
	public static int formatNameBytes(String name, byte[] buf,
			final int offset, final int length) {
		try {
	 ...
	 */
	public static int formatNameBytes(String name, byte[] buf,
			final int offset, final int length, final ZipEncoding encoding)
			throws IOException {
		int len = name.length();
	 ...
	 */
	public static void formatUnsignedOctalString(final long value,
		...
		if (value == 0) {
			...
			if (val != 0) {
				throw new IllegalArgumentException(value + "="
						...
						+ " will not fit in octal number buffer of length "
						+ length);
			}
		}
	 ...
	 * string with leading zeros. The converted number is followed by space and
	 * NUL
	 *
	 * @param value
	 *            The value to write
	 ...
	 */
	public static int formatOctalBytes(final long value, byte[] buf,
			final int offset, final int length) {
	 ...
	 *             if the value (and trailer) will not fit in the buffer
	 */
	public static int formatLongOctalBytes(final long value, byte[] buf,
			final int offset, final int length) {
	...
	/**
	 * Write an long integer into a buffer as an octal string if this will fit,
	...
	public static int formatLongOctalOrBinaryBytes(final long value,
		...
		final long maxAsOctalChar = length == TarConstants.UIDLEN ? TarConstants.MAXID
	 ...
	 *
	 * Uses {@link #formatUnsignedOctalString} to format the value as an octal
	 * string with leading zeros. The converted number is followed by NUL and
	 * then space.
	 *
	 * @param value
