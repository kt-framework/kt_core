...
public class TarInputStream extends FilterInputStream {
	...
	/**
	 * This contents of this array is not used at all in this class, it is only
	 * here to avoid repreated object creation during calls to the no-arg read
	 * method.
	 */
	 ...
	 * current entry. Integer.MAX_VALUE is returned in case more than
	 * Integer.MAX_VALUE bytes are left in the current entry in the archive.
	 *
	...
	public long skip(long numToSkip) throws IOException {
		...
		long skip = numToSkip;
		while (skip > 0) {
			int realSkip = (int) (skip > SKIP_BUF.length ? SKIP_BUF.length
					: skip);
		...
		if (currEntry != null) {
			...
			if (debug) {
				System.err.println("TarInputStream: SKIP currENTRY '"
						+ currEntry.getName() + "' SZ " + entrySize + " OFF "
						+ entryOffset + "  skipping " + numToSkip + " bytes");
			}
	...
	/**
	 * Get the next record in this tar archive. This will skip over any
	 * remaining data in the current entry, if there is one, and place the input
	 * stream at the header of the next entry. If there are no more entries in
	 * the archive, null will be returned to indicate that the end of the
						...
						if (ch == '=') { // end of keyword
							...
							if (got != len - read) {
								throw new IOException("Failed to read "
										+ "Paxheader. Expected " + (len - read)
										+ " bytes, read " + got);
							}
							// Drop trailing NL
							String value = new String(rest, 0, len - read - 1,
									"UTF-8");
	...
	private void applyPaxHeadersToCurrentEntry(Map<String, String> headers) {
		/*
		 * The following headers are defined for Pax. atime, ctime, charset:
		 * cannot use these without changing TarEntry fields mtime comment gid,
		 * gname linkpath size uid,uname SCHILY.devminor, SCHILY.devmajor: don't
		 * have setters/getters for those
		 */
	...
	private void readGNUSparse() throws IOException {
		/*
		 * we do not really process sparse files yet sparses = new ArrayList();
